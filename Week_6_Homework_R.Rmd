...
ANSWER HERE: Tibble 1 would be the best one since the means of both X and Y are taking into account time. Tibble 2 and Tibble 3 don't account for time in their X variables. Panel data is needed for Fixed Effects, and Tibble 1 gives that to us. 
...
```{r}
my_df <- tib1 %>% mutate(category = factor(individual))
```
...
```{r}
fe_demean <- lm(Y ~ X + individual, data = my_df)
export_summs(fe_demean)
# figure out if should use X_within and Y_within, and if 'individual' should be included
```
...
```{r}
export_summs(fe_demean, fe_lsdv, fe_robust, digits = 3, coefs = c('individual', 'X'))
```

i. Explain why the R squared is so much lower in Model 1 than in Model 2.

ANSWER HERE: Model 1 R^2 is looking at the Within Variation, leaving the Fixed Effects out of the model. Model 2 uses the Fixed Effects in its model. The two are both correct, they're just measuring two different things. 
